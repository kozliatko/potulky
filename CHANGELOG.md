...
### Fixed
- **CI bežal na Node 20, produkcia (Dockerfile) už na Node 26** — `better-sqlite3@13.x` vyžaduje Node `>=22`, takže dependabot bump na túto verziu padal v CI s `Worker exited unexpectedly` (natívny modul sa nedal na Node 20 nabootovať), hoci lokálne aj v Docker builde fungoval bez problémov. `actions/setup-node` v `build.yml` zjednotený na Node 26 pre backend aj frontend job, nech CI reálne odráža produkčné prostredie
