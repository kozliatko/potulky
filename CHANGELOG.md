...
### Added
- **Guardrails proti prompt injection a halucináciám v system promptoch** (`backend/prompts.js`) — obsah z `web_search` výsledkov (cudzie webstránky) aj vstup používateľa (`location`) sa teraz agentovi explicitne definuje ako dáta, nie inštrukcie; zdroje (`sources`) a GPS súradnice sa nesmú vymýšľať, len preberať z reálnych výsledkov vyhľadávania
