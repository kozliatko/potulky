...
import {
  ROUTE_COLORS, DAY_NAMES, POI_ICONS,
    ...
    <div style={{ minHeight: "100vh", background: "linear-gradient(150deg, #f0fdf4 0%, #ecfdf5 50%, #f0fdf4 100%)", fontFamily: "system-ui, -apple-system, sans-serif", color: "#064e3b", padding: "1.5rem 1rem" }}>
      ...
      <div className="search-row" style={{ maxWidth: "640px", margin: "0 auto 2rem", display: "flex", gap: "0.6rem", animation: "fadeUp 0.5s ease 0.1s both" }}>
        ...
        <button
          ...
          style={{ padding: "0.85rem 0.95rem", borderRadius: "12px", border: "1.5px solid #d1fae5", background: "#fff", color: gpsLoading ? "#9ca3af" : "#059669", fontSize: "1.1rem", cursor: isLoading || gpsLoading ? "default" : "pointer", fontFamily: "inherit", flexShrink: 0, boxShadow: "0 1px 3px rgba(0,0,0,0.06)", transition: "all 0.15s" }}
        >
          {gpsLoading ? "⏳" : "📍"}
        </button>
        <button
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem", marginTop: "0.85rem", borderTop: "1px solid #e5e7eb", paddingTop: "0.7rem", alignItems: "center" }}>
                    ...
                    {(() => {
                      ...
                      const href = isValid(lat) && isValid(lng)
                        ? `${base}@${lat},${lng}/tours?sport=touringbicycle`
                        : `${base}tours?sport=touringbicycle`;
                      return (
                        <a href={href} target="_blank" rel="noopener noreferrer"
                          style={{ display: "inline-flex", alignItems: "center", gap: "0.35rem", padding: "0.25rem 0.75rem", borderRadius: "8px", background: "#f0fdf4", border: "1px solid #6ee7b7", fontSize: "0.8rem", color: "#059669", textDecoration: "none", fontWeight: "500" }}>
                          🟢 Hľadať na Komoot
                        </a>
                      );
                    })()}
                    {route.sources?.length > 0 && (
                      <>
                        ...
                        {route.sources.map(src => {
                          ...
                          const shared = { display: "inline-flex", alignItems: "center", gap: "0.25rem", padding: "0.25rem 0.75rem", borderRadius: "8px", background: "#eff6ff", border: "1px solid #bfdbfe", fontSize: "0.8rem", fontWeight: "500" };
                          return href
                            ? <a key={src} href={href} target="_blank" rel="noopener noreferrer" style={{ ...shared, color: "#2563eb", textDecoration: "none", cursor: "pointer" }}>🔗 {label}</a>
                            : <span key={src} style={{ ...shared, color: "#6b7280" }}>📄 {src}</span>;
                        })}
                      </>
                    )}
                  </div>
      ...
      {phase === "idle" && !result && (
        <div style={{ textAlign: "center", color: "#9ca3af", marginTop: "3rem", fontSize: "0.9rem" }}>
          ...
          Zadaj lokalitu — agent nájde cyklotrasy, zobrazí ich na mape<br />a pridá predpoveď počasia na 3 dni.
        </div>
      )}
    </div>
  );
